    ...
    for index in range(start_train_path_index, args.epochs * len(train_paths)):
        ...
        for batch_idx, input_data in enumerate(train_dataloader):
            ...
            if not (total_steps % args.checkpoint_freq):
                ...
                # フルモデルの保存追加
                full_model_path = os.path.join(args.checkpoint_dir, f"full_total_steps{total_steps}")
                smp.save_checkpoint(full_model_path, tag="fullmodel.pt", partial=False, model=model, user_content=user_content)
...
def parse_args():
    ...
    io_grp.add_argument(
        ...
        default=os.environ["SM_CHANNEL_CHECKPOINT"],
...
def main():
    ...
    if args.load_full or args.load_partial:
        ...
        # 追記部分
        print(f"load_full:{args.load_full}, load_partial]{args.load_partial}")
        if args.load_full:
            import tarfile
            model_tar_file_path = os.path.join(os.environ["SM_CHANNEL_MODEL"], "model.tar.gz")
            with tarfile.open(model_tar_file_path) as t:
                t.extractall(os.environ["SM_CHANNEL_MODEL"])
            increment_model_dir = os.environ["SM_CHANNEL_MODEL"]
        # path = args.checkpoint_dir if partial else args.model_dir
        path = args.checkpoint_dir if partial else increment_model_dir   
